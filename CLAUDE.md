...
This import is pulled into every main session — it is how Claude knows the project state, rules, and key files. Generated at every SessionStart by the hook script from `CLAUDE.md` + `.claude/context/*.md` + `.claude/lessons.md` + `.claude/session-log.md` (last 3 entries). Missing file is skipped silently.

@.claude/context-minimal.md

> **Full context for subagents:** Subagents that need complete architecture, tech-stack, and coding-standards detail (code-reviewer, architecture-reviewer, pr-reviewer, test-writer, migration-writer, refactoring-agent) read `.claude/agent-context.md` via their Prerequisite line — also generated by the SessionStart hook. The main session loads only the minimal summary; subagents load what they need.
